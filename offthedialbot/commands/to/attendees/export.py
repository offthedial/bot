...
async def main(ctx):
    ...
    profiles: list = utils.dbh.profiles.find({"meta.competing": True}, {"_id": True})
    file = create_file(ctx, profiles)
...
def create_file(ctx, profiles: list):
    ...
    for profile in profiles:
        user = ctx.bot.get_user(profile["_id"])
